...
Part III: Testing
----------------------------------
In order to test the operating system, a test.c file has been included with the operating system files.  The main.c file utilizes this file when running the operating system automatically, so place it in the project along with the other files when using the monitor program.   The provided main.c creates a FIFO, writes to both of it, then reads from it.  It then creates two processes, test and test2 which are functions contained within test.c.  These are called with OS_Create and the first parameter that they are given is the length of time that the function should loop.  These test processes make the green LEDs on the board blink in two different patterns and for two different lengths of time. 
